...
class img2text(object):
    def __init__(self, appId = '18255707', apiKey = 'cZ0xCDGdjmzHG4AZl5Y3IsKa', secretKey = 'yBBpgWnWZReE9LaD4nRGPHSgWeuQNrlv'):
        self.ocr             = AipOcr(appId = appId, apiKey = apiKey, secretKey = secretKey)
        self.default_options = {"language_type" : "CHN_ENG", "detect_direction" : "true", "detect_language" : "true", "probability" : "true"}
    def np2img(arr):
        return np.array(cv2.imencode('.jpg', arr)[1]).tobytes()
    def get_result(self, img, options = None):
        if options == None:
            options = self.default_options
        img  = img2text.np2img(img)
        res  = self.ocr.basicGeneral(img, options)
        text = ''
...
if __name__ == '__main__':
    test = img2text()
    img  = cv2.imread('2020-01-04_205631.jpg')
    print(test.get_result(img))
